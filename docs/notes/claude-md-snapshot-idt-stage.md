...
## 2. Trạng thái hiện tại (cập nhật lần cuối: giai đoạn IDT/ISR)
...
| PIC remap + PIT timer | ❌ Chưa làm | **Bẫy quan trọng**: PIC mặc định đè lên vector 8-15 (trùng CPU exception) |
| E820 memory map | ❌ Chưa làm | Bootloader đang identity-map cứng 1GB mà **không hỏi BIOS RAM thật có bao nhiêu** |
| Physical Frame Allocator | ❌ Chưa làm | Cần E820 map trước |
| Virtual Memory Manager (thật) | ⚠️ Chỉ có bản thô | Hiện tại: identity-map cứng 1GB, không có `map()/unmap()` linh hoạt |
...
│   │                                 # (hiện vẫn để phẳng trong kernel/, di
│   │                                 #  chuyển vào đây là việc dọn dẹp riêng)
...
│   │   ├── pmm.hpp / pmm.cpp        # [SẮP CÓ] Physical Frame Allocator
...
1. **PIC chưa remap — BẪY khi bật hardware interrupt.** PIC 8259 mặc định
   ...
   `sti`, nếu không timer tick sẽ trông giống double fault giả.
2. **Không có E820 memory map.** Bootloader identity-map cứng 1GB mà không
   hỏi BIOS RAM thật tồn tại ở đâu (`INT 15h, EAX=0xE820`, phải gọi ở Real
   Mode trước khi vào Protected Mode). Physical Frame Allocator sẽ cấp nhầm
   vùng không tồn tại/bị chiếm nếu thiếu bước này.
3. **Chưa có serial (COM1) debug output.** Đang in chẩn đoán qua VGA text
   buffer thủ công (80x25, không copy-paste được). Nên thêm driver serial
   sớm để log qua `qemu -serial stdio`.
4. **Chưa có `kprintf`/`panic()` dùng chung.** Mỗi chỗ báo lỗi tự viết lại
   logic in hex/text — nên gom thành một hàm chung trước khi viết thêm
   nhiều subsystem báo lỗi khác nhau.

## 8. Lộ trình phát triển tiếp theo (theo đúng thứ tự phụ thuộc)

```
PIC remap + PIT timer (mở khoá hardware interrupt an toàn, nền cho scheduler)
   ↓
E820 memory map (bootloader, Real Mode) — bắt buộc trước Physical Frame Allocator
   ↓
Physical Frame Allocator (mm/pmm) — bitmap/free-list frame 4KB, dựa trên E820
   ↓
